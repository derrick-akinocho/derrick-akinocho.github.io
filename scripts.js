...
class MenubarNavigation {
  ...
  onMenuitemClick(event) {
    ...
    if (this.hasPopup(tgt)) {
      ...
      this.updateContent(tgt.href);
      this.closePopupAll();
      showSection(tgt.textContent.trim().toLowerCase());
    }
    event.stopPropagation();
    event.preventDefault();
  }
...
window.addEventListener('DOMContentLoaded', (event) => {
  ...
  function showSectionFromUrl() {
    ...
    if (fragment === 'about' || fragment === 'blog' || fragment === 'contact') {
      // Hide all sections
...
document.getElementById('contactForm').addEventListener('submit', function(event) {
  ...
  if (isValid) {
    ...
    window.location.href = 'index.html';  // Remplacez par la page d'accueil réelle
  }
});